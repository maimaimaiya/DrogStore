...
def MergeData(username,cookie,update_path):
    file_path = path[0]+"/data/"+username+".xls"
    ...
    # print(df_inner)
    #保存到本地
    df_inner.to_excel(path[0]+"/data/"+username+"_merge.xls", index=False)
...
def GetOnlineData(username,id_multiple):
    ...
    #查找 商城价格 < 编号价格*倍数
    # print(df)
    df.to_excel(path[0]+"/data/" + username + "_price.xls", index=False)
    print("筛选商品")
    # return df

#修改对应价格到网站
def UpdatePrice(username,cookie,multiple,select_list):
    file_path = path[0]+"/data/" + username + "_price.xls"
    df = DataFrame(read_excel(file_path))
    cow_num = df.shape[0]
